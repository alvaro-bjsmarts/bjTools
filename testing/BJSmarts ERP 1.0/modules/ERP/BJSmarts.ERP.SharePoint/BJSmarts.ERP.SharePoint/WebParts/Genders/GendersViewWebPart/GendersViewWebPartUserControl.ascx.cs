...
namespace BJSmarts.ERP.SharePoint.WebParts.Genders.GendersViewWebPart
{
    public partial class GendersViewWebPartUserControl : UserControl
    {
        ...
        public SPListItem GetItemByBdcId()
        {
            ...
            if (Page.Request.QueryString["RecordId"] != null)
            {
                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        SPList list = web.Lists["Genders"];

                        foreach (SPListItem externalItem in list.Items)
                        {
                            if (int.Parse(externalItem["GenderId"].ToString()) == RecordId)
                            {
                                item = externalItem;
                            }
                        }
        ...
        {
            return Microsoft.SharePoint.Utilities.SPUtility.GetLocalizedString("$Resources: " + strInput, "GendersResourceFile", (uint)intLCID);
        }
        ...
        protected void b_Click(object sender, EventArgs e)
        {
            if (Page.Request.QueryString["RecordID"] != null)
            {
                Response.Redirect("/Pages/GendersUpdatePage.aspx?RecordId=" + Page.Request.QueryString["RecordID"].ToString());
            }
        }
