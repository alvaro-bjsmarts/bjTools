...
namespace BJSmarts.ERP.SharePoint.WebParts.Organization.OrganizationsUpdateWebPart
{
    public partial class OrganizationsUpdateWebPartUserControl : UserControl
    {
                    ...
                    SPSecurity.RunWithElevatedPrivileges(delegate()
                    {
                        using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                        {
                            using (SPWeb web = site.OpenWeb())
                            {
                                ...
                                ddlCurrencyId.SelectedIndex = GetItemIndex(ListItem, ddlCurrencyId, "CurrencyId");

                                LoadingLookupOneToOne("Industry", ddlIndustryId, "Name", "Sort_Order", "IndustryId", true, intLCID);
                                ddlIndustryId.SelectedIndex = GetItemIndex(ListItem, ddlIndustryId, "IndustryId");
        ...
        public SPListItem GetItemByBdcId()
        {
            ...
            if (Page.Request.QueryString["RecordId"] != null)
            {
                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        SPList list = web.Lists["Organizations"];

                        foreach (SPListItem externalItem in list.Items)
                        {
                            if (int.Parse(externalItem["OrganizationId"].ToString()) == RecordId)
                            {
                                item = externalItem;
                            }
                        }
        ...
        private void addRecord(SPUser author, SPWeb web, Boolean IsDraft)
        {
            ...
            SPListItemCollection listItems = web.Lists["Organizations"].Items;
            ...
            item["CurrencyId"] = Convert.ToInt64(ddlCurrencyId.SelectedValue);

            item["IndustryId"] = Convert.ToInt64(ddlIndustryId.SelectedValue);
            if (intLCID == 1033)
            {
                item["Language"] = 0;
            }
