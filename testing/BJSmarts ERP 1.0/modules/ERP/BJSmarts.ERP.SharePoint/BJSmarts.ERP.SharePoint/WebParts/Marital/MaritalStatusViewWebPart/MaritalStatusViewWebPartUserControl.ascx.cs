...
namespace BJSmarts.ERP.SharePoint.WebParts.Marital.MaritalStatusViewWebPart
{
    public partial class MaritalStatusViewWebPartUserControl : UserControl
    {
        ...
        public SPListItem GetItemByBdcId()
        {
            ...
            if (Page.Request.QueryString["RecordId"] != null)
            {
                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        SPList list = web.Lists["Marital Status"];

                        foreach (SPListItem externalItem in list.Items)
                        {
                            if (int.Parse(externalItem["MaritalStatusId"].ToString()) == RecordId)
                            {
                                item = externalItem;
                            }
                        }
        ...
        {
            return Microsoft.SharePoint.Utilities.SPUtility.GetLocalizedString("$Resources: " + strInput, "MaritalStatusResourceFile", (uint)intLCID);
        }
        ...
        protected void b_Click(object sender, EventArgs e)
        {
            if (Page.Request.QueryString["RecordID"] != null)
            {
                Response.Redirect("/Pages/MaritalStatusUpdatePage.aspx?RecordId=" + Page.Request.QueryString["RecordID"].ToString());
            }
        }
