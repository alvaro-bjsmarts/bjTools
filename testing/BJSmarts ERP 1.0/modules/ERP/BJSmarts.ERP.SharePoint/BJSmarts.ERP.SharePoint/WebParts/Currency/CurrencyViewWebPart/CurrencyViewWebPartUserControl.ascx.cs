...
namespace BJSmarts.ERP.SharePoint.WebParts.Currency.CurrencyViewWebPart
{
    public partial class CurrencyViewWebPartUserControl : UserControl
    {
        ...
        public SPListItem GetItemByBdcId()
        {
            ...
            if (Page.Request.QueryString["RecordId"] != null)
            {
                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        SPList list = web.Lists["Currency Type"];

                        foreach (SPListItem externalItem in list.Items)
                        {
                            if (int.Parse(externalItem["CurrencyTypeId"].ToString()) == RecordId)
                            {
                                item = externalItem;
                            }
                        }
        ...
        {
            return Microsoft.SharePoint.Utilities.SPUtility.GetLocalizedString("$Resources: " + strInput, "CurrencyTypeResourceFile", (uint)intLCID);
        }
        ...
        protected void b_Click(object sender, EventArgs e)
        {
            if (Page.Request.QueryString["RecordID"] != null)
            {
                Response.Redirect("/Pages/CurrencyTypeUpdatePage.aspx?RecordId=" + Page.Request.QueryString["RecordID"].ToString());
            }
        }
