...
{
    public partial class IndustryViewWebPartUserControl : UserControl
    {
        ...
        public SPListItem GetItemByBdcId()
        {
            ...
            if (Page.Request.QueryString["RecordId"] != null)
            {
                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        SPList list = web.Lists["Industry"];

                        foreach (SPListItem externalItem in list.Items)
                        {
                            if (int.Parse(externalItem["IndustryId"].ToString()) == RecordId)
                            {
                                item = externalItem;
                            }
                        }
        ...
        {
            return Microsoft.SharePoint.Utilities.SPUtility.GetLocalizedString("$Resources: " + strInput, "IndustryResourceFile", (uint)intLCID);
        }
        ...
        protected void b_Click(object sender, EventArgs e)
        {
            if (Page.Request.QueryString["RecordID"] != null)
            {
                Response.Redirect("/Pages/IndustryUpdatePage.aspx?RecordId=" + Page.Request.QueryString["RecordID"].ToString());
            }
        }
